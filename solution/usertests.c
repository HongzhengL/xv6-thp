...
void sbrktest(void) {
  ...
  oldbrk = sbrk(0, VMALLOC_SIZE_BASE);

  // can one sbrk() less than a page?
  a = sbrk(0, VMALLOC_SIZE_BASE);
  int i;
  for (i = 0; i < 5000; i++) {
    b = sbrk(1, VMALLOC_SIZE_BASE);
    if (b != a) {
...
#define BIG (100 * 1024 * 1024)
  ...
  *lastaddr = 99;
  ...
  // can one de-allocate?
  a = sbrk(0, VMALLOC_SIZE_BASE);
  c = sbrk(-4096, VMALLOC_SIZE_BASE);
  if (c == (char *)0xffffffff) {
    printf(stdout, "sbrk could not deallocate\n");
    exit();
  }
  c = sbrk(0, VMALLOC_SIZE_BASE);
  if (c != a - 4096) {
  ...
  a = sbrk(0, VMALLOC_SIZE_BASE);
  c = sbrk(4096, VMALLOC_SIZE_BASE);
  if (c != a || sbrk(0, VMALLOC_SIZE_BASE) != a + 4096) {
  ...
  a = sbrk(0, VMALLOC_SIZE_BASE);
  c = sbrk(-(sbrk(0, VMALLOC_SIZE_BASE) - oldbrk), VMALLOC_SIZE_BASE);
  if (c != a) {
  ...
  for (i = 0; i < sizeof(pids) / sizeof(pids[0]); i++) {
    if ((pids[i] = fork()) == 0) {
      ...
      sbrk(BIG - (uint)sbrk(0, VMALLOC_SIZE_BASE), VMALLOC_SIZE_BASE);
      write(fds[1], "x", 1);
  ...
  if (sbrk(0, VMALLOC_SIZE_BASE) > oldbrk)
    sbrk(-(sbrk(0, VMALLOC_SIZE_BASE) - oldbrk), VMALLOC_SIZE_BASE);
...
void argptest() {
  ...
  read(fd, sbrk(0, VMALLOC_SIZE_BASE) - 1, -1);
  close(fd);
